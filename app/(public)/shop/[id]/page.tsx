...
export default function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleAddToCart = () => {
    ...
    // Simulate API or Cart Context delay
    setTimeout(() => {
  ...
  return (
    <div className="bg-surface-container-lowest min-h-screen pb-24 pt-24 md:pt-32">
      <div className="container-custom max-w-6xl">
        ...
        <Link href="/shop" className="inline-flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors font-label-md text-xs uppercase tracking-widest mb-8">
        ...
        <div className="bg-surface rounded-[40px] border border-outline-variant/30 p-4 md:p-8 shadow-sm">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            {/* Left: Image */}
            <div className="rounded-[32px] overflow-hidden bg-surface-container h-[400px] md:h-[600px] relative">
              ...
              <div className="absolute top-6 left-6">
                <span className="bg-white/90 backdrop-blur-md text-primary font-label-md text-xs uppercase tracking-widest px-4 py-2 rounded-full shadow-sm">
                  {product.category}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col justify-center py-4">
              ...
              <h1 className="font-display-sm text-4xl md:text-5xl text-on-surface mb-6">
                {product.name}
              </h1>
              ...
              <div className="mt-auto">
                {product.isAffiliate ? (
                  <a 
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-primary to-[#2a5a3b] text-white py-4 rounded-2xl font-label-md text-sm uppercase tracking-widest hover:opacity-90 transition-all shadow-lg active:scale-[0.98]"
                  >
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-label-md text-sm uppercase tracking-widest transition-all shadow-lg ${
                      added 
                      ? 'bg-green-600 text-white border border-green-600'
                      : 'bg-gradient-to-r from-primary to-[#2a5a3b] text-white hover:opacity-90 active:scale-[0.98]'
                    }`}
                  >
                    {addingToCart ? (
                      <><Loader2 className="w-5 h-5 animate-spin" /> Wird hinzugefügt...</>
                    ) : added ? (
                      <><Check className="w-5 h-5" /> Im Warenkorb (Demo)</>
                    ) : (
                ...
                {!product.isAffiliate && (
                  <p className="text-center mt-4 text-[10px] text-on-surface-variant font-body-sm uppercase tracking-wider">
                    Hinweis: Dies ist eine Demo-Umgebung. Der Checkout ist noch nicht aktiv.
                  </p>
                )}
              </div>
